...
import { MessageCircle, MapPin, Calendar, BookOpen, Users, Search, Bot, GraduationCap, Clock, Bell } from 'lucide-react';
const CampusAssistant = () => {
  ...
  const quickActions = ['Find the library', 'What\'s for lunch today?', 'Show my class schedule', 'Campus events this week'];
            ...
            <div className="max-w-2xl mx-auto">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl border border-white/20 p-6 shadow-glow">
                ...
                <div className="flex gap-3">
                  <Input placeholder="e.g., Where is the computer science building?" value={chatMessage} onChange={e => setChatMessage(e.target.value)} className="bg-white/10 border-white/20 text-white placeholder:text-white/70" />
                  <Button className="bg-white text-primary hover:bg-white/90" onClick={() => setChatMessage('')}>
                    Ask
                  </Button>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {quickActions.map((action, index) => <Button key={index} variant="outline" size="sm" className="bg-white/5 border-white/20 text-white hover:bg-white/10 text-xs" onClick={() => setChatMessage(action)}>
                      {action}
                    </Button>)}
                </div>
              </div>
      ...
      <section className="py-16 px-4">
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => <Card key={index} className="group hover:shadow-card transition-all duration-300 hover:-translate-y-1 border-border/50 bg-gradient-card">
